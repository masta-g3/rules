...
`pi/skills/focus` provides `/skill:focus <ticket-id>` for autonomous work on an approved plan, feature, or task. During `/execute`, the agent can also call `start_focus` when approved in-scope work is likely to require multiple turns without immediate user input. Focus delegates implementation behavior to `execute`, while `extensions/workflow-runtime/` owns workflow state, the pulsing `FOC ✦` / `FOC ✧` rail, and guarded continuation delivery. Every completed turn automatically produces a compact, expandable continuation event reminding the agent to re-read the active plan when one exists and keep working; there is no turn limit or response marker. The agent ends the loop by calling `end_focus` with a `completed` or `blocked` outcome and summary. Manual user input also stops focus mode, compaction preserves it, and reload, restart/resume, new session, or fork requires explicit reinvocation.
