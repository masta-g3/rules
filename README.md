...
- `/session-name refresh` regenerates the native Pi name. `/session-metadata-status` reports the latest metadata operation, model, latency, parsed result, attempts, failures, and skipped models. `/wf-todos` or `Ctrl+Alt+T` opens a fresh read-only plan checklist drawer.
