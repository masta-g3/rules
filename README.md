...
- The footer shows `◇ meta` when metadata is ready or succeeded, `◆ meta` while a request runs, `◇! meta` after a timeout, unsupported-model, provider, response, or parse failure, and `◇× meta` after an authentication failure. The extension remains active in failure states; only authentication failures direct the user to `/login openai-codex`.
