...
This is a positional cue, not an execution audit. Invoking a later step directly checks every earlier position. The current marker is `◉` while active and `✓` after Plan, Review, or Reflect publishes its terminal activity. Full rails show glyph plus stage label (`✓ Plan ─ ◉ Execute`); narrow rails show `N/5 <marker> <short>`. Focus mode replaces the active Execute marker and display with pulsing `◆ Focus` / `◇ Focus`. After `/commit` finishes all required repository commits and feature/plan closeout, the guarded `complete_workflow` tool retains `✓✓✓✓✓` and `Commit complete`. A new step or ticket, `/wf-clear`, or manual dismissal replaces it. Failed or blocked Commit turns remain active.

The runtime also owns the workflow's ordered display definition. Every persisted `workflow-runtime` custom entry includes the five step ids, short codes, and friendly labels alongside the active state and producer `updatedAt` state-change timestamp. While focus is active, the same entry adds a generic `activeMode` display (`FOC`, `Focus`, and `turn N`). Execute-scoped focus keeps Execute as the base step; standalone focus has no base workflow step. Ending focus omits the mode. Consumers such as dashboards can render the producer's order and optional mode without mirroring vocabulary or reading private execution state, while missing or malformed metadata affects display only. Reloading or resuming an active historical workflow writes one normalized entry with the current definition; definition-current entries and inactive sessions do not incur that normalization write.
...
- `/wf-ticket <ticket-id>` sets or overrides the active ticket manually, publishes generic `pi-agent-hub-context`, and names Pi from the ticket title. That title stays fixed across workflow stages; manual and agent renames cannot replace it while the ticket is linked.
- `/wf-ticket clear` unlinks the ticket and removes its plan display while retaining the workflow step, activity, and completion marker. `/wf-clear` clears both the workflow and ticket link. Both stop Focus continuation, clear old task attention, and release name protection without aborting the current response. The current name, ticket files, and backlog status stay unchanged; the cleared link stays cleared on resume.
- `/session-name refresh` keeps a linked ticket's title or generates a name for an unlinked session. `/session-metadata-disable` stops optional naming and attention model calls for the current session; `/session-metadata-enable` starts them again. `/session-metadata-status` reports whether metadata is disabled or the latest operation, model, latency, parsed result, attempts, failures, skipped models, and bounded rejected output with retry guidance for parse failures. `/wf-todos` or `Ctrl+Alt+T` opens a fresh read-only plan checklist drawer.
...
- A valid `ask_user_question` tool start immediately publishes explicit `question` attention with a deterministic request ID and a bounded first-question summary plus `+N more`; matching tool completion clears only that request. Reload/resume clears request-backed questions because the original questionnaire cannot survive the process boundary. After completed turns, one bounded Spark-first call may publish explicit `ready`, `question`, or `blocked` attention. It clears before the next run. Naming uses the same resolver but a separate prompt and output contract. Metadata calls disable reasoning, allow five seconds per model, and stop trying a model for the rest of the session after the API reports it unsupported.
- The workflow rail shows `◇– meta` when metadata is disabled, `◇ meta` when it is ready or succeeded, `◆ meta` while a request runs, `◇! meta` after a timeout, unsupported-model, provider, response, or parse failure, and `◇× meta` after an authentication failure. With no active workflow, the badge remains in the same widget by itself. The extension remains active in failure states; only authentication failures direct the user to `/login openai-codex`.
- Double-press `ctrl+shift+right` to run the next workflow skill (with the active ticket when set); on `commit`, the same double-press clears only the indicator and keeps the ticket linked. Ignored while Pi is busy or the editor contains unsent text.
- Native Pi forks and Hub's **Fork and compact** clear inherited workflow and ticket context, releasing the child's name without changing the original session. The task reset survives resume. Use matching updated Hub and installed Rules extensions; Hub requires reset confirmation before compaction and reports an error if Rules cannot provide it. Updating files does not reload an already-running Hub dashboard or Pi extension. Ordinary Hub `f` retains the ticket link and its name protection.
...
Run fixture tests without inherited Hub project-root or fork markers; otherwise ticket reads can target the launching session's repository instead of the fixture.

```bash
env -u PI_AGENT_HUB_PRIMARY_CWD -u PI_AGENT_HUB_FORK_COMPACT uv run pytest -q
```
