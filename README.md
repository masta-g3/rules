...
This is a positional cue, not an execution audit. Invoking a later step directly checks every earlier position. The current marker is `◉` while active and `✓` after Plan, Review, or Reflect publishes its terminal activity. Full rails show glyph plus stage label (`✓ Plan ─ ◉ Execute`); narrow rails show `N/5 <marker> <short>`. Focus mode replaces the active Execute display with compact `Focus`. After `/commit` finishes all required repository commits and feature/plan closeout, the guarded `complete_workflow` tool retains `✓✓✓✓✓` and `Commit complete`. A new step or ticket, `/wf-clear`, or manual dismissal replaces it. Failed or blocked Commit turns remain active.
