...
- `/wf-ticket <ticket-id>` sets or overrides the active ticket manually, publishes generic `pi-agent-hub-context`, and names Pi from the ticket title. That title stays fixed across workflow stages; manual and agent renames cannot replace it while the ticket is linked. `/wf-clear` clears only the workflow rail, not the ticket link or its name protection.
...
- Native Pi forks and Hub's **Fork and compact** clear inherited workflow and ticket context, releasing the child's name without changing the original session. The task reset survives resume. Use matching updated Hub and installed Rules extensions. Ordinary Hub `f` uses CLI startup semantics and retains the ticket link and its name protection.
