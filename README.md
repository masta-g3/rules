...
`pi/skills/focus` provides `/skill:focus <ticket-id>` for autonomous work on an approved plan, feature, or task. During `/execute`, the agent can also call `start_focus` when approved in-scope work is likely to require multiple turns without immediate user input. Focus delegates implementation behavior to `execute`, while `extensions/workflow-runtime/` owns workflow state, the pulsing `FOC ✦` / `FOC ✧` rail, and guarded continuation delivery. Every turn ending normally (`stop`) automatically produces a compact, expandable continuation event reminding the agent to re-read the active plan when one exists and keep working; there is no turn limit or response marker. Esc/abort, provider errors, output limits, and other non-normal stops leave focus active but paused without scheduling another turn; ordinary user input resumes it. The agent ends the loop by calling `end_focus` with a `completed` or `blocked` outcome and summary. Changing workflow steps stops focus, compaction preserves it, and reload, restart/resume, new session, or fork requires explicit reinvocation.
