...
Work directly from the active plan we have been discussing. If a planning document is available, iterate through each phase incrementally—implement, test, and seek user feedback before moving to the next phase. Mark completed steps with `[x]` and update the plan if scope or approach shifts.
