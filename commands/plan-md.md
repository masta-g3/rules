...
3. Write out a detailed implementation plan with precise details (code snippets, file names, etc.) on all the steps needed, including a detailed architectural layout that maps components, data flows, and dependencies. Think of this as a implementation guide for a junior engineer who will execute on the job. For substantial features (new apps, rewrites, multi-service work), the plan should be robust (aim for +300 lines, including pseudocode, diagrams and breakdowns as needed); for minor edits or narrow fixes keep it as concise as the change warrants. If a plan for a broad request lands under that scale, expand it further (more diagrams, API contracts, UX flows, edge cases) until it comfortably clears the +300-line bar. Structure it like a narrative PRD: flowing prose, subsections, tables, and diagrams as the spine, with bullet lists used only for short enumerations.
