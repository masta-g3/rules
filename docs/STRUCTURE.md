...
- **One workflow runtime**: `extensions/workflow-runtime/index.ts` owns Pi lifecycle wiring, persisted workflow state, the rail, and continuation delivery; dependency-free transitions and queue guards stay in `core.ts` for direct behavioral testing. The runtime is also the producer of the ordered workflow display contract (`id`, `short`, and optional `label` per step): every persisted custom entry carries the current definition and its producer timestamp, and active historical entries are normalized once on reload/resume so consumers never need a mirrored step list.
