...
- **One workflow runtime**: `extensions/workflow-runtime/index.ts` owns Pi lifecycle wiring, persisted workflow state, the rail, and continuation delivery; dependency-free transitions and queue guards stay in `core.ts` for direct behavioral testing. The runtime is also the producer of the workflow display contract: every persisted custom entry carries the ordered step definition (`id`, `short`, and optional `label`) and producer state-change timestamp. Focus additionally projects a generic optional `activeMode` (`id`, `short`, optional `label`/`detail`) while preserving Execute as the base step; consumers do not need mirrored vocabulary or private execution state. Active historical entries are normalized once on reload/resume so consumers never need a mirrored step list.
